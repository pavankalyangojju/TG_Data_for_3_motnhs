...
#define IR_SENSOR_PIN 25 // Define the IR sensor pin
...
void readIRSensor() {
    ...
    // Control the LED based on the IR sensor state
    digitalWrite(LED_PIN, irDetected ? HIGH : LOW);
    
    // Send IR detection state to Blynk
    Blynk.virtualWrite(V5, irDetected ? 1 : 0);
...
void sendSensor() {
    ...
    // Read IR sensor and control LED
    readIRSensor();
}
